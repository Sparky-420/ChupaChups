...

{
    ...
    const currentOption = '<option value="cagada">Receta actual (predeterminada)</option>';
    const oldOption = '<option value="viejo">Receta del viejo (alternativa)</option>';
    assert.ok(html.includes(currentOption));
    assert.ok(html.includes(oldOption));
    assert.ok(html.indexOf(currentOption) < html.indexOf(oldOption));
}

{
    const { elements } = makeApp();
    assert.equal(elements.receta_select.value, 'cagada');
    assert.equal(elements.peso_carne.value, '0');
}

{
    ...
    elements.peso_carne.value = '20';
    context.calcular();
    includesAll(elements.resultados.innerHTML, ['Receta del viejo (alternativa)', '0.300', '300.0', '0.040', '40.0', '0.080', '80.0', '0.100', '100.0']);
...
for (const oldWeight of [19.9, 20]) {
    const savedData = JSON.stringify({ peso_carne: oldWeight });
    const { context, elements } = makeApp(savedData);
    assert.equal(elements.peso_carne.value, String(oldWeight));
